    ...
    with mp_holistic.Holistic(min_detection_confidence=0.5, min_tracking_confidence=0.5) as holistic:
        ...
        while cap.isOpened():
            ...
            if len(sequence) == 35:
                ...
                # print("unique predictions in 15 frame : ",np.unique(predictions[-15:]))
                # print("max predictions in 15 frame    : ",np.unique(predictions[-15:])[0])
...
def mk_sentence(
    ...
    prompt = "다음의 단어들을 순서대로 이용해서해서 자연스러운 문장 하나로 만들어줘. 예를 들어 주어진 단어가(나 오늘 금요일 기분 좋다)라면 (나는 오늘이 금요일이라 기분이 좋아)라고 자연스럽게 만들어줘. 이때 새로운 단어를 마음대로 넣지 않았으면 좋겠고, 물음표가 제시되는 경우 의문문으로 만들어줘. " + ",".join(words)
